...
class Client{
	...
	function getReport()
	{
		...
		if (is_array($result) && count($result)>0)
		{
			if (is_array($result[0]))
			{
				foreach($result as $reportArray)
					$ret1[] = new StatusReport($reportArray);	
			}else{
				$ret1[] = new StatusReport($result);
			}
...
class StatusReport{
	...
	function StatusReport(&$ret1=array())
	{
		$this->errorCode = $ret1[errorCode];
		$this->reportStatus = $ret1[reportStatus];
		$this->serviceCodeAdd = $ret1[serviceCodeAdd];
		$this->mobile = $ret1[mobile];
		$this->submitDate = $ret1[submitDate];
		$this->receiveDate = $ret1[receiveDate];
		$this->seqID = $ret1[seqID];
